...
\begin{itemize}
	...
	\item \textbf{Experiencias de AR compartidas}.-Las aplicaciones de AR ya no están limitadas a una sola persona o dispositivo, actualmente \textbf{ARKit 2} proporciona experiencias multiusuario como juegos donde las experiencias de realidad aumentada se extienden hasta poder ser participe como un espectador.\cite{B20}
...
Los métodos que utiliza ARToolKit para visualizar la realidad aumentada (AR) están limitados, esta API necesita escanear una tarjeta con una marca.\cite{B07}\par
ARToolKit es un ToolKit proporciona funciones predefinidas que debe llamar en un orden específico para desarrollar un software con realidad aumentada (AR),también se puede usar ToolKit por separado. ARToolKit es compatible con múltiples plataformas. ARToolKit utiliza algunas extensiones para su correcto funcionamiento que a continuación se mencionan.
